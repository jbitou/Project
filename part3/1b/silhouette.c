...
double compute_silhouette(pcluster clusters, user *users, int numofitems, int numofusers, int k) {
	...
	for (i=0; i < k; i++) {
		...
		while (object != NULL) {
			...
			if (users[id1].userId == users[c].userId) {
				object = object->next;
				continue;
			}
			suma = 0;
			...
			while (temp != NULL) {
				id2 = temp->position;
				if (users[id1].userId == users[id2].userId) {
					temp = temp->next;
					continue;
				}
				if (metric == 1)		suma += distance_Euclidean(users[id1].ratings,users[id2].ratings,numofitems);
				else if (metric == 2) suma += distance_Cosine(users[id1].ratings,users[id2].ratings,numofitems);
				temp = temp->next;
			}
			...
			for (j=0; j < k; j++) {
				if (users[idnext].userId == users[clusters[j].center].userId) {
					s = j;
					break;
				}
			}
			...
			while (second != NULL) {
				id2 = second->position;
				if (metric == 1)		sumb += distance_Euclidean(users[id1].ratings,users[id2].ratings,numofitems);
				else if (metric == 2) sumb += distance_Cosine(users[id1].ratings,users[id2].ratings,numofitems);
				second = second->next;
			}
