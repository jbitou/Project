...
double compute_silhouette(pcluster clusters, double **vectors, int r, int numConform, int k) {
	int i, j, z, id1, id2, s, compare;
	double si, a, b, sum, suma, sumb, n, max, average, total = 0.0, totalsum = 0.0, *c, *nextbest;
	pointp object, temp, second;
	/**For every cluster**/
	for (i=0; i < k; i++) {
		object = clusters[i].items;
		c = clusters[i].center.vector;	
		sum = average = 0;
		/**For each object of this cluster**/
		while (object != NULL) {
			temp = clusters[i].items;
			/**For each object i (comformation) of this cluster**/
			id1 = object->position;
			suma = 0;
			...
			while (temp != NULL) {
				...
				suma += distance_Euclidean(vectors[id1],vectors[id2],r);
				temp = temp->next;
			}
			...
			for (j=0; j < k; j++) {
				compare = 0;
				for (z=0; z < r; z++) {
					if (nextbest[z] == clusters[j].center.vector[z]) compare++;
				}
				if (compare == r) {
					s = j;
					break;
			...
			while (second != NULL) {
				id2 = second->position;
				sumb += distance_Euclidean(vectors[id1],vectors[id2],r);
				second = second->next;
			}
